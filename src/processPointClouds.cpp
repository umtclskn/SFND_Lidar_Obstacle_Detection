...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::euclideanCluster(typename pcl::PointCloud<PointT>::Ptr cloud, KdTree* tree, float distanceTol, int minSize, int maxSize)
{
    ...
    while(index < cloud->points.size())
    {
		if(processed[index] == false)
		{
            ...
            clusterHelper(index, cloud, cluster_indexes, processed, tree, distanceTol);

            if(cluster_indexes.size() >= minSize && cluster_indexes.size() <= maxSize)
            {
                for(int i = 0; i < cluster_indexes.size(); i++)
                {
                    PointT point;
                    point = cloud->points[cluster_indexes[i]];
                ...
                for(int i = 1; i < cluster_indexes.size(); i++)
                {
                    processed[cluster_indexes[i]] = false;
                }
            }
		}
        index++;
